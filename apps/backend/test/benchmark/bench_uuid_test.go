...
func BenchmarkCreateUuid(b *testing.B) {
	...
	repo := repository.NewGormUuidRepository(db)
...
func BenchmarkGetUuids(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUuidRepository(db)
...
func BenchmarkUpdateUuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUuidRepository(db)
...
func BenchmarkDeleteUuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUuidRepository(db)
