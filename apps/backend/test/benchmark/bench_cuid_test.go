...
func BenchmarkCreateCuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormCuidRepository(db)
...
func BenchmarkGetCuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormCuidRepository(db)

	// Create a test user
	testUser := model.UserCUID{
		UserBase: &model.UserBase{
			UserName:  "getuseroeu",
			FirstName: "Get",
			LastName:  "User",
...
func BenchmarkGetCuids(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormCuidRepository(db)
...
func BenchmarkUpdateCuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormCuidRepository(db)
...
func BenchmarkDeleteCuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormCuidRepository(db)
