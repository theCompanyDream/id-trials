...
func BenchmarkCreateKsuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormKsuidRepository(db)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		user := model.UserKSUID{
...
func BenchmarkGetKsuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormKsuidRepository(db)

	// Create a test user
	testUser := model.UserKSUID{
		UserBase: &model.UserBase{
			UserName:  "getuseroeu",
			FirstName: "Get",
			LastName:  "User",
...
func BenchmarkGetKsuids(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormKsuidRepository(db)
...
func BenchmarkUpdateKsuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormKsuidRepository(db)
...
func BenchmarkDeleteKsuid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormKsuidRepository(db)
