...
func BenchmarkCreateUlid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUlidRepository(db)
...
func BenchmarkGetUlid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUlidRepository(db)

	// Create a test user
	testUser := model.UserUlid{
		UserBase: &model.UserBase{
			UserName:  "getuseroeu",
			FirstName: "Get",
			LastName:  "User",
...
func BenchmarkGetUlids(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUlidRepository(db)
...
func BenchmarkUpdateUlid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUlidRepository(db)
...
func BenchmarkDeleteUlid(b *testing.B) {
	db := setup.NewPostgresMockDB()
	repo := repository.NewGormUlidRepository(db)
